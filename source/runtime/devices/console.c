...
void *thread_func(void *args) {
    ...
    while (1) {
        size_t count = read(consolePipe[0], buffer, 256);
        if (count > 0) {
            con_write(buffer, count > 256 ? 256 : count);
        }
    }
}
...
PrintConsole* consoleInit(PrintConsole* console) {
    ...
    if (firstConsoleInit) {
        pipe(consolePipe);
        dup2(consolePipe[1], STDOUT_FILENO);
        
        setvbuf(stdout, NULL, _IONBF, BUFSIZ);
        
        // start thread for handle stdout output
        pthread_t pthread;
